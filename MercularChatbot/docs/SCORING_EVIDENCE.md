...
Latest release check (2026-08-30): 296 tests passed; the local NLP corpus passed
all 177 cases; local p95 parse/retrieve/Flex latency was 171.022 ms; and the
...
- One-on-one events start LINE's five-second loading animation in the background;
  unsupported chats or loading-endpoint failures never delay or suppress the reply.
...
- The installed Rich Menu sends its catalogue commands to the same category-picker
  and recommendation flow as ordinary user messages.
- The navy/cyan visual system from the supplied MercuMate artwork is reused by
  the Rich Menu and product action color.

Evidence:

```bash
pytest -q tests/test_line_views.py tests/test_app.py
```
