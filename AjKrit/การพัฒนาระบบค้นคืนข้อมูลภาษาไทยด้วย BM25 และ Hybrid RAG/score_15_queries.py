...
# CONFIG
# ------------------------------------------------------------
JSON_FILE = Path("thai_qa_utf8.json")
CSV_FILE = Path("thai_qa_paraphrase_15.csv")

MODEL_NAME = "intfloat/multilingual-e5-small"
RRF_K = 60

OUTPUT_FILE = Path("thai_qa_paraphrase_15_scored.csv")
EMBED_CACHE = Path("thai_instruction_e5_embeddings.npy")
...
if not JSON_FILE.exists():
    raise FileNotFoundError(
        f"ไม่พบ {JSON_FILE.resolve()}"
    )

if not CSV_FILE.exists():
    raise FileNotFoundError(
        f"ไม่พบ {CSV_FILE.resolve()}"
    )
