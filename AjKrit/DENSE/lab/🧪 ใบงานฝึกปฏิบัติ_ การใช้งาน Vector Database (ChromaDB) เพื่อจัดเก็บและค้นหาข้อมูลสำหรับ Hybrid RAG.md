...
| def search\_vector\_db(query\_text, n\_results=2):     results \= collection.query(         query\_texts=\[query\_text\],         n\_results=n\_results     )     return results \# ทดสอบ Q1 และ Q2 จากใบงานเดิม queries \= \[     "การขอเบิกเงินค่าเดินทางไปทำงานต่างจังหวัด", \# Q1 (Synonym/Context)     "ขอคู่มือรหัส D-9902"                          \# Q2 (Exact Code Match) \] for q in queries:     print(f"\\n🔍 Query: '{q}'")     res \= search\_vector\_db(q)     for doc, meta, dist in zip(res\['documents'\]\[0\], res\['metadatas'\]\[0\], res\['distances'\]\[0\]):         print(f"  \-\> \[{meta\['doc\_id'\]}\] (Code: {meta\['doc\_code'\]}) Distance: {dist:.4f} | Text: {doc}") |
| :---- |

### **📘 Step 5: แก้ไขปัญหา Exact Match ด้วย Metadata Filtering**
